/* ============================================================================
   ...
   A complete Dragon Quest 1 text adventure that encodes all game state
   into chat messages for persistence.

   SAVE FORMAT (printed in every response):
   [SAVE: HP=20/20|MP=0/10|LVL=3|GOLD=120|EXP=45|LOC=TANTEGEL_CASTLE|
    INV=HERB,TORCH|EQP=COPPER_SWORD,LEATHER_ARMOR|KEYS=0|
...
let APPLY_LIMIT = 20;    // Higher limit for complex game logic
...
// Locations in Dragon Quest 1
const LOCATIONS = {
    "tantegel_castle": {
        name: "Tantegel Castle",
        desc: "The majestic castle of King Lorik. Stone walls tower above you.",
        exits: { south: "tantegel_throne", east: "tantegel_courtyard" }
    },
    "tantegel_throne": {
        name: "Throne Room",
        desc: "King Lorik XVI sits upon his throne, looking worried.",
        exits: { north: "tantegel_castle", down: "tantegel_basement" }
    },
    "tantegel_courtyard": {
        name: "Castle Courtyard",
        desc: "A peaceful courtyard with a healing spring.",
        exits: { west: "tantegel_castle", south: "tantegel_gate" }
    },
    "tantegel_gate": {
        name: "Castle Gate",
        desc: "The southern gate leads out into the wilderness.",
        exits: { north: "tantegel_courtyard", south: "overworld" }
    },
    "overworld": {
        name: "Alefgard Overworld",
        desc: "The vast wilderness of Alefgard stretches before you. Monsters roam freely.",
        exits: { north: "tantegel_gate", west: "brecconary", south: "swamp", east: "mountains" }
    },
    "brecconary": {
        name: "Town of Brecconary",
        desc: "A small town near the castle. Shops and inns line the streets.",
        exits: { east: "overworld", north: "weapon_shop", south: "item_shop" }
    },
    "garinham": {
        name: "Town of Garinham",
        desc: "A larger town to the east. You can see a graveyard nearby.",
        exits: { west: "overworld", north: "graveyard" }
    },
    "rimuldar": {
        name: "Town of Rimuldar",
        desc: "A prosperous town with many travelers.",
        exits: { north: "overworld" }
    },
    "cantlin": {
        name: "Town of Cantlin",
        desc: "The largest town, surrounded by mountains.",
        exits: { south: "overworld" }
    },
    "charlock_castle": {
        name: "Charlock Castle",
        desc: "The dark fortress of the Dragonlord. Evil emanates from within.",
        exits: { up: "charlock_throne" }
    }
};

// Items and equipment
const ITEMS = {
    "herb": { type: "consumable", effect: "heal", power: 30, price: 24 },
    "torch": { type: "consumable", effect: "light", power: 1, price: 8 },
    "fairy_water": { type: "consumable", effect: "repel", power: 1, price: 38 },
    "wings": { type: "consumable", effect: "escape", power: 1, price: 70 },
    "dragon_scale": { type: "consumable", effect: "defense", power: 2, price: 0 },
    ...
    "magic_armor": { type: "armor", defense: 24, price: 7700 },
    "erdricks_armor": { type: "armor", defense: 28, price: 0 },
...
// Monster database
const MONSTERS = {
    "slime": { hp: 3, attack: 5, defense: 2, gold: 2, exp: 1, zone: "early" },
    "red_slime": { hp: 4, attack: 7, defense: 3, gold: 3, exp: 2, zone: "early" },
    "drakee": { hp: 6, attack: 9, defense: 6, gold: 5, exp: 3, zone: "early" },
    "ghost": { hp: 7, attack: 11, defense: 8, gold: 8, exp: 4, zone: "early" },
    "magician": { hp: 13, attack: 15, defense: 12, gold: 18, exp: 13, zone: "mid" },
    "scorpion": { hp: 20, attack: 18, defense: 16, gold: 26, exp: 16, zone: "mid" },
    "druin": { hp: 22, attack: 20, defense: 18, gold: 30, exp: 18, zone: "mid" },
    "metal_slime": { hp: 4, attack: 10, defense: 255, gold: 6, exp: 115, zone: "special" },
    "knight": { hp: 37, attack: 40, defense: 40, gold: 70, exp: 42, zone: "late" },
    "magiwyvern": { hp: 49, attack: 56, defense: 50, gold: 105, exp: 58, zone: "late" },
    "demon_knight": { hp: 47, attack: 60, defense: 54, gold: 110, exp: 78, zone: "late" },
    "dragonlord_1": { hp: 100, attack: 90, defense: 75, gold: 0, exp: 0, zone: "boss" },
    "dragonlord_2": { hp: 165, attack: 140, defense: 90, gold: 0, exp: 2000, zone: "boss" }
};

// Spells
const SPELLS = {
    "heal": { mp: 4, effect: "heal", power: "17-25", learn: 3 },
    "hurt": { mp: 2, effect: "attack", power: "5-12", learn: 4 },
    "sleep": { mp: 2, effect: "sleep", power: 1, learn: 7 },
    "radiant": { mp: 3, effect: "light", power: 1, learn: 9 },
    "stopspell": { mp: 2, effect: "seal", power: 1, learn: 10 },
    "outside": { mp: 6, effect: "escape", power: 1, learn: 12 },
    "return": { mp: 8, effect: "warp", power: 1, learn: 13 },
    "repel": { mp: 2, effect: "repel", power: 1, learn: 15 },
    "healmore": { mp: 10, effect: "heal", power: "85-100", learn: 17 },
    "hurtmore": { mp: 5, effect: "attack", power: "58-65", learn: 19 }
};
...
const FLAGS = [
    ...
    "STAFF_OF_RAIN",
    "STONES_OF_SUNLIGHT",
    "ERDRICKS_ARMOR_FOUND",
    "ERDRICKS_SWORD_FOUND",
    "RAINBOW_DROP",
    "DRAGONLORD_DEFEATED",
    "GAME_WON"
];
...
const ENTITY_DB = {
    "hero": {
        gender: "M",
        aliases: ["player", "protagonist", "you", "warrior"],
        lore: []
    },
    "king_lorik": {
        gender: "M",
        aliases: ["king", "lorik", "king lorik"],
        lore: []
    },
    "princess_gwaelin": {
        gender: "F",
        aliases: ["princess", "gwaelin", "lady"],
        lore: []
    },
    "dragonlord": {
        gender: "M",
        aliases: ["dragon lord", "master of darkness"],
        lore: []
    }
};
...
/* ============================================================================
   [SECTION] ENGINE CODE (Same as v14)
   DO NOT MODIFY
   ========================================================================== */
//#region ENGINE

// [All the engine code from the original v14 system goes here]
// Including: OUTPUT_GUARDS, INPUT_NORMALIZATION, UTILITIES,
// COMPILATION, SELECTION_PIPELINE, APPLY+SHIFTS, FLUSH
...
const WINDOW_DEPTH = 10; // Increased for better save state detection
...
function wordGatesPass(e) {
  const any = arr(e && e.andAny);
  const all = arr(e && e.andAll);
  const none = arr(e && e.notAny);
  if (any.length && !any.some(w => hasTerm(last, w))) return false;
  if (all.length && !all.every(w => hasTerm(last, w))) return false;
  if (none.length && none.some(w => hasTerm(last, w))) return false;
  return true;
}
...
function isAlwaysOn(e) {
  return !(e && (e.keywords || e.tag || e.minMessages != null || e.maxMessages != null));
}
...
for (const [i, e] of _ENGINE_LORE.entries()) {
  const hit = isAlwaysOn(e) || getKW(e).some(kw => hasTerm(last, kw));
...
for (const idx of selected) {
  ...
  if (e.Shifts) {
    for (const sh of e.Shifts) {
      const hit = isAlwaysOn(sh) || getKW(sh).some(kw => hasTerm(last, kw));
