/* ============================================================================
   DRAGON QUEST 1 TEXT ADVENTURE - FULLY FIXED VERSION
   Author: Icehellionx (adapted)

   CRITICAL FIXES IN THIS VERSION:
   ✅ Derived tags from SAVE state (fixes persistent state tracking)
   ✅ Fixed combat/death/location gates (now use tags, not user input)
...
//#region GLOBAL_KNOBS
let DEBUG       = 0;
let APPLY_LIMIT = 15;  // Reduced from 25 to prevent instruction overload

/* ============================================================================
   [SECTION] SAVE STATE PARSER (IMPROVED)
   ========================================================================== */
//#region SAVE_PARSER
...
function parseSaveState() {
    ...
    const saveStr = saveMatch[1].replace(/\s+/g, ' ').trim();  // Normalize whitespace
    const state = {};

    // Split by | but only at top level (not in nested structures)
...
// Validate and repair SAVE (ensure required keys exist)
...
const EXITS = {
    ...
    OVERWORLD: { north: "OVERWORLD", south: "OVERWORLD", east: "OVERWORLD", west: "OVERWORLD" },  // Simplified
    CHARLOCK_ENTRANCE: { north: "CHARLOCK_THRONE", south: "OVERWORLD" },
    CHARLOCK_THRONE: { south: "CHARLOCK_ENTRANCE" }
};

/* ============================================================================
   [SECTION] DERIVED TAGS FROM SAVE STATE (THE CRITICAL FIX!)
   ========================================================================== */
//#region DERIVED_TAGS
...
function deriveTags() {
    ...
    if (inCombat()) {
        ...
        // Parse combat enemy for specific tags
        ...
        // Parse enemy HP for death/phase transitions
    ...
    // 2. Location tags (normalized to lowercase for matching)
    ...
    // 3. FLAGS as tags (persistent state!)
    ...
    // 5. Status field tags (shop state, etc)
...
/* ============================================================================
   [SECTION] GAME LOGIC ENTRIES (FIXED WITH TAG-BASED GATES!)
   ========================================================================== */
//#region AUTHOR_ENTRIES
const dynamicLore = [

// ═══════════════════════════════════════════════════════════════════════════
// GAME START (fixed to trigger on first turn)
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// CORE COMMANDS (fixed gates)
// ═══════════════════════════════════════════════════════════════════════════

  {
    keywords: ["stats", "status"],
    priority: 5,
    andAllTags: ["not_in_combat"],  // FIXED: tag-based, not user input
...
Include complete SAVE block at end of output.`,
...

  {
    ...
    priority: 4,
    andAllTags: ["not_in_combat"],  // FIXED
    personality: `Hero searches area. Based on {LOC} from SAVE state, describe what they find:

Location-specific details from LOCATION_DESCRIPTIONS and current LOC tag:
...
- OVERWORLD: Wilderness, visible landmarks (Ref: https://dragonquest.fandom.com/wiki/Alefgard)

Check EXITS table for available directions.
...
- King Lorik: "Please save the Princess from the cave!" (Ref: https://dragonquest.fandom.com/wiki/King_Lorik)
...
- (Ref: https://dragonquest.fandom.com/wiki/Erdrick)
...
// ═══════════════════════════════════════════════════════════════════════════
// MOVEMENT SYSTEM (properly gated)
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// MONSTER INFORMATION (Individual lookups - preserved)
// ═══════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════
// RANDOM ENCOUNTERS (FIXED - now uses derived tags!)
// ═══════════════════════════════════════════════════════════════════════════

  {
    tag: "traveled",  // Triggered by movement
    andAllTags: ["zone_overworld"],  // FIXED: uses derived location tag
    priority: 4,
    probability: 0.3,
...
Reference the wiki link from MONSTERS[monster_name].wiki for additional lore.
...
// ═══════════════════════════════════════════════════════════════════════════
// COMBAT SYSTEM (FIXED with proper gates and damage clamping)
// ═══════════════════════════════════════════════════════════════════════════

  {
    keywords: ["attack", "fight", "strike"],
    andAllTags: ["combat_active"],  // FIXED: tag-based
    priority: 5,
...
3. Hero damage = MAX(1, ({STR} + weapon_ATK) / 2 + random(0-4) - enemy_DEF / 2)  // Min 1 damage
4. Extract enemy current HP from COMBAT field (parse format: MONSTER_NAME_HP##)
5. New enemy HP = MAX(0, old_HP - damage)  // Can't go below 0
...
- Enemy attacks back
- Enemy damage = MAX(1, (enemy_ATK - hero_DEF) / 2 + random(0-4))  // Min 1 damage
...
Include updated SAVE block.`,
...

  {
    ...
    andAllTags: ["combat_active"],  // FIXED
    priority: 5,
...
Reference: Gather information about Dragon Quest spells from https://dragonquest.fandom.com/wiki/List_of_spells_in_Dragon_Quest`,
...

  {
    ...
    andAllTags: ["in_town"],  // FIXED: tag-based
    priority: 5,
...
Include updated SAVE block with restored HP/MP and reduced GOLD.`,
...

  {
    ...
    andAllTags: ["loc_tantegel_courtyard"],  // FIXED: tag-based
    priority: 5,
...
// ═══════════════════════════════════════════════════════════════════════════
// SHOPPING SYSTEM (FIXED with STATUS persistence)
// ═══════════════════════════════════════════════════════════════════════════

  {
    keywords: ["weapon shop", "buy weapon"],
    andAllTags: ["in_town"],  // FIXED
    priority: 5,
    triggers: ["in_shop"],
...
Show current gold. Set STATUS=SHOP_ITEM. Include SAVE block.`,
  ...
  {
    keywords: ["buy "],
    andAllTags: ["in_shop"],  // FIXED: requires shop state tag
    priority: 5,
...
// ═══════════════════════════════════════════════════════════════════════════
// QUEST PROGRESSION (FIXED - tag-based persistence!)
// ═══════════════════════════════════════════════════════════════════════════

  {
    keywords: ["princess", "gwaelin", "rescue"],
    andAny: ["cave"],  // Can keep this as keyword match for cave exploration
    notAnyTags: ["flag_princess_saved"],  // FIXED: check FLAG tag
    priority: 5,
    triggers: ["PRINCESS_SAVED"],  // Adds flag on success
...
Reference: Gather information about Princess Gwaelin from https://dragonquest.fandom.com/wiki/Princess_Gwaelin`,
  ...
  {
    keywords: ["talk", "king"],
    andAllTags: ["flag_princess_saved", "loc_tantegel_throne"],  // FIXED: both tags required
    notAnyTags: ["flag_gwaelins_love_received"],  // FIXED
    priority: 5,
...
Reference: Gather information about Gwaelin's Love from https://dragonquest.fandom.com/wiki/Gwaelin%27s_Love`,
...

  {
    ...
    andAny: ["swamp"],  // Keyword match is okay for this special search
    notAnyTags: ["flag_erdricks_token"],  // FIXED
    priority: 4,
    probability: 0.5,
...
// ═══════════════════════════════════════════════════════════════════════════
// FINAL BOSS (FIXED phase transitions)
// ═══════════════════════════════════════════════════════════════════════════

  {
    keywords: ["dragonlord", "approach", "confront"],
    andAllTags: ["loc_charlock_throne", "flag_rainbow_drop"],  // FIXED
    notAnyTags: ["flag_dragonlord_defeated"],  // FIXED
    priority: 5,
...
Reference: Gather information about the Dragonlord from https://dragonquest.fandom.com/wiki/Dragonlord`,
  ...
  {
    tag: "combat_action",  // Triggers when Dragonlord 1 is defeated
    andAllTags: ["enemy_dragonlord", "enemy_defeated"],  // FIXED: tag-based detection
    notAnyTags: ["flag_dragonlord_phase2"],  // Only trigger once
    priority: 5,
...
Include updated SAVE block.`,
  ...
  {
    tag: "combat_end",  // Triggers when any combat ends
    andAllTags: ["flag_dragonlord_phase2"],  // FIXED: only if phase 2 was reached
    priority: 5,
...
// ═══════════════════════════════════════════════════════════════════════════
// LEVELING & DEATH (FIXED)
// ═══════════════════════════════════════════════════════════════════════════

  {
    tag: "combat_end",  // Triggers when combat ends
    priority: 5,
...
Include updated SAVE block.`,
  ...
  {
    andAllTags: ["hp_zero"],  // FIXED: tag-based death detection!
    priority: 5,
    triggers: ["hero_died"],
...
/* ============================================================================
   [SECTION] ENGINE CODE (v14 + DERIVED TAGS FIX)
   ========================================================================== */
//#endregion
//#region ENGINE
...
// ═══════════════════════════════════════════════════════════════════════════
// THE CRITICAL FIX: Derive tags from SAVE BEFORE selection!
// ═══════════════════════════════════════════════════════════════════════════

const DERIVED_TAGS = deriveTags();

// Merge derived tags with trigger tags
...
// Phase 1: Direct keyword and prev.keyword hits (NOW WITH DERIVED TAGS!)
for (const [i, e] of _ENGINE_LORE.entries()) {
  ...
  if (!hit || !entryPasses(e, allTags)) continue;  // Use allTags!
  buckets[prio(e)].push(i);
  picked[i] = 1;
  getTrg(e).forEach(t => addTag(allTags, t));  // Add to allTags
}

// Phase 2: Tag-triggered entries (NOW WITH DERIVED TAGS!)
for (const [i, e] of _ENGINE_LORE.entries()) {
  if (picked[i] || !(e.tag && hasTag(allTags, e.tag))) continue;
  if (!entryPasses(e, allTags)) continue;  // Use allTags!
  buckets[prio(e)].push(i);
  picked[i] = 1;
